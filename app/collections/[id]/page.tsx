...
// Merge dynamic cards.json entries into the hardcoded collection
const dynamicCards = cards.filter(card => COLLECTION_ART[card.collection]);

dynamicCards.forEach(card => {
  COLLECTION_ART[card.collection].push({
    name: card.name,
    type: card.type,
    image: card.image,        // correct key
    glow: "#fff",
    description: card.description, // correct key
...
useEffect(() => {
  ...
  if (activeEmote) return; // ⭐ Prevent hijacking ESC navigation
    ...
    // ⭐ Overscroll DOWN at bottom → next collection
    if (atBottom && e.deltaY > 0 && nextSlug) {
      attempts++;

      const stretch = -Math.min(Math.sqrt(attempts) * 10, 35);
      setNudge(stretch);
    ...
    // ⭐ Overscroll UP at top → previous collection
    ...
    // ⭐ Normal scrolling → reset rubber band
    attempts = 0;
    setNudge(0);
  };
  ...
  const handleEsc = (e: KeyboardEvent) => {
    ...
    if (activeEmote) {
      setActiveEmote(null);
      console.log("ESC id:", id);
      console.log("ESC rawParams:", rawParams);
      console.log("ESC params:", params);
...
const cardParam = searchParams.get("card");
  ...
  const visibleCards = COLLECTION_ART[art.slug];
	  // ⭐ Deep-link: open modal when ?card= is in the URL
useEffect(() => {
  if (!cardParam) return;

  // Find matching card in this collection
  const match = COLLECTION_ART[art.slug].find(
  ...
  // Special banana mode
  if (match.name.toLowerCase() === "banana") {
  } else {
  ...
  <div className="collections-page">
    ...
    {activeEmote && (
      <div
        ...
        onClick={() => {
          setActiveEmote(null);
          router.push(`/collections/${art.slug}`, { scroll: false });
        }}
      >
        <div
          className="emote-modal emote-layout"
        ...
        >
          ...
          {activeEmote.type === "Emote" && (
            <ProfileCard
              ...
              behindGlowColor={activeEmote.glow}
              iconUrl={null}
              behindGlowEnabled
          ...
          {/* ⭐ Splash / Wallpaper / Sticker */}
          {(activeEmote.type === "Mobile Wallpaper" ||
  activeEmote.type === "Splash Art" ||
  activeEmote.type === "Sticker") && (
  <div className="art-layout">
    <img
      className="full-wallpaper"
      src={activeEmote.image}
      alt={activeEmote.name}
    />
  </div>
)}
          ...
          <div className="emote-info-boxes">
            ...
            {/* ⭐ Render About box ONLY if description exists */}
            {activeEmote.description &&
            ...
            {/* ⭐ Render Details box ONLY if details OR link exist */}
            {(activeEmote.details &&
              activeEmote.details.trim() !== "") ||
            (activeEmote.link &&
              activeEmote.link.trim() !== "") ? (
              <div className="emote-info-box">
                {activeEmote.details &&
                  activeEmote.details.trim() !== "" && (
                    <p>{activeEmote.details}</p>
                  )}

                {activeEmote.link &&
                  activeEmote.link.trim() !== "" && (
                    <a
                      href={activeEmote.link}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="emote-link"
                    >
                      {activeEmote.link}
                    </a>
                  )}
      ...
      >
        {visibleCards.map(piece => (
          <ProfileCard
            ...
            enableTilt={true}
            enableMobileTilt={false}
            behindGlowColor={piece.glow}
            iconUrl={null}
            behindGlowEnabled
            innerGradient="linear-gradient(145deg,#60496e8c 0%,#71C4FF44 100%)"
            onContactClick={() => {
              const slugified = piece.name
                .toLowerCase()
                .replace(/\s+/g, "-");
